...
struct TicTacToe3D {
  ...
  bool is_done() { return player_won_game(CLIENT) || player_won_game(SERVER); }
  ...
  bool check_is_valid(int i_big, int j_big) {
    ...
    if (field[k].sm[i_small][j_small] != NO_PLAYER ||
        macroboard.sm[i_macro][j_macro] != NO_PLAYER) {
      // cout << "small is busy\n";
      return false;
    }

    // celula pe care o vreau eu este in cea 3x3 in care trebuie
    if ((i_macro == next_x && j_macro == next_y) &&
        macroboard.sm[next_x][next_y] == NO_PLAYER) {
      // cout << "cell is in target and target is not conq\n";
      return true;
    }

    // celula pe care o vreau eu NU este in cea 3x3 in care trebuie, dar cea
    // care trebuie e FULL
    if (!(i_macro == next_x && j_macro == next_y) &&
        macroboard.sm[next_x][next_y] != NO_PLAYER) {
      // cout << "cell is not in target and but target is conq" << i_macro << "
      // " << j_macro << " " << next_x << " " << next_y <<   "\n";
      return true;
    }
  ...
  int get_val(int i_big, int j_big) {
    ...
    // cout << i_big << ' ' << j_big << ':' << k << ':' <<  i_small << ' ' <<
    // j_small << ':'  << i_macro << ' ' << j_macro << '\n';
    // cout << "\n";
