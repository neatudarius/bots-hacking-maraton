...
std::string read_message(conn_t conn, headers_t &headers,
  ...
  if (headers.find("Content-Length") != headers.end()) {
    ...
    body.insert(body.end(), body_part.c_str(),
                body_part.c_str() + body_part.length());
...
void run_server(int port, std::function<void(conn_t)> client_handler) {
  ...
  int enable = 1;
  if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (const char *)&enable,
                 sizeof(int)) < 0)
    error();
  ...
  while (true) {
    ...
    client_list.emplace_back(
        [client_fd, &client_list, &client_handler, &client_lock]() {
          try {
