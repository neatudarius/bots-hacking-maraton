...
pair cell::getBig() const { return pair(lin_big, col_big); }

pair cell::getSmall() const { return pair(lin_small, col_small); }

pair cell::getMacro() const { return pair(lin_macro, col_macro); }
...
cell::cell(int i, int j) {
  ...
  //                                             0)); // none
  //                                          // always check if (diag_small &&
  //                                          exp(diag_small))
  //                                          // before use diag_small in
  //                                          expression
  ...
  //                                             0)); // none
  //                                          // always check if (diag_macro &&
  //                                          exp(diag_macro))
  //                                          // before use diag_macro in
  //                                          expression
}

/** check if I can move in this cell **/
bool cell::canBePicked(const int macroboard[][3], const int field[][9]) {
  return macroboard[lin_macro][col_macro] == -1 && !field[lin_big][col_big];
}
