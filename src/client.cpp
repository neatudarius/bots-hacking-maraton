...
struct Client {
  ...
  int rounds = 0;
  bool done = false;
  string initial_msg = "486f77206d7563682074696d652064696420776520706c617920544"
                       "f4745544845523f2028696e2068656164657220706c656173652069"
                       "6e7365727420726f756e64733a2076616c756529";

  Client(char *server, int port) { conn = create_connection(server, port); }
  ...
  inline void move(int i_big, int j_big) {
    ...
    if (game.check_is_valid(i_big, j_big)) {
      ...
      if (game.is_done()) {
        ...
        std::vector<uint8_t> req_body;
        write_request(conn, "/final-request-you-won", {}, initial_msg.c_str(),
                      initial_msg.size());
      } else {
        // cout << "CLIENT:\n";
        find_my_move();
        ...
        if (game.is_done()) {
          ...
          std::vector<uint8_t> req_body;
          write_request(conn, "/final-request-I-won", {}, initial_msg.c_str(),
                        initial_msg.size());
        } else {
          std::vector<uint8_t> req_body;
          write_request(conn, "/valid-move", {}, req_body.data(),
                        (int)req_body.size());
        }
      }
    } else {
      ...
      std::vector<uint8_t> req_body;
      write_request(conn, "/invalid-move", {}, req_body.data(),
                    (int)req_body.size());
    }
  }
};
  ...
  while (true) {
    ...
    if (command == "get_version") {
      ...
      if (no_message == true) {
        ...
        write_request(client.conn, "/lets_play_tictactoe", headers,
                      req_body.data(), (int)req_body.size());
      }
    }
  }
